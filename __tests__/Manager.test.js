const Manager = require('../lib/Manager.js');
const Employee = require('../lib/Employee.js');

test('should create Manager object', () => {
    const expected = new Manager('1', '2', '3', '4');
    expect(expected instanceof Manager).toBe(true);
})

test('should create Manager object which is also Employee', () => {
    const expected = new Manager('1', '2', '3', '4');
    expect(expected instanceof Employee).toBe(true);
})

test('should create obj with Name', () => {
    const testValue = 'Adam'
    const expected = new Manager('testValue', '', '', '');
    expect(expected.name).toBe('testValue');
})

test('should create obj with Id', () => {
    const testValue = '1'
    const expected = new Manager('', 'testValue', '', '');
    expect(expected.id).toBe('testValue');
})

test('should create obj with Email', () => {
    const testValue = 'email@email.com'
    const expected = new Manager('', '', 'testValue', '');
    expect(expected.email).toBe('testValue');
})

test('should create obj with office number', () => {
    const testValue = 'UC Davis'
    const expected = new Manager('', '', '', 'testValue');
    expect(expected.officeNUmber).toBe('testValue');
})

test('should create obj with Role', () => {
    const expected = new Manager('', '', '', '');
    expect(expected.getRole()).toBe('Manager');
})